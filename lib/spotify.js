...
export const getAllPlaylists = async () => {
  ...
  for(let i = 0; i < MAX_PLAYLISTS; i += offset ) {
      ...
      if (items != null) {
        ...
        items.forEach((playlist, index) => playlists.push({
            index:          index + i,
            id:             playlist.id,
  ...
  playlists = playlists.filter(playlist => nameFilter(playlist.name))
...
export const getAllSongData = async (tracks) => {
  ...
  for(let i = 0; i < (tracks.length / 100); i++){
    ...
    for(let j = 0; j < 100 && (j + (i * 100)) < tracks.length; j++) {
      contributors[tracks[[j + (i * 100)]].added_by.href] = (contributors[tracks[[j + (i * 100)]].added_by.href] || 0) + 1
  ...
  const userResponse = await spotifyFetch(Object.keys(contributors).reduce((a, b) => contributors[a] > contributors[b] ? a : b))
  const topContributor = await userResponse.json()
